...
grid.innerHTML = `<div class="grid-corner"></div>${weekdays.map((day) => `<div class="day-heading">${day}</div>`).join("")}`;
for (let hour = 8; hour < 23; hour += 1) {
  grid.insertAdjacentHTML("beforeend", `<div class="hour-label">${String(hour).padStart(2, "0")}:00</div>${weekdays.map(() => '<div class="hour-cell"></div>').join("")}`);
}
for (const block of blocks.filter((item) => item.isRecurring || (item.startsAt.slice(0, 10) >= dashboard.dataset.weekStart && item.startsAt.slice(0, 10) <= dashboard.dataset.weekEnd))) {
  const [startHour, startMinute] = block.startTime.split(":").map(Number);
  const [endHour, endMinute] = block.endTime.split(":").map(Number);
  const start = (startHour - 8) * 60 + startMinute;
  const duration = (endHour * 60 + endMinute) - (startHour * 60 + startMinute);
  if (start < 0 || start > 900) continue;
  const item = document.createElement("div");
  item.className = `preview-block source-${block.source}`;
  item.style.gridColumn = String(block.weekday + 2);
  item.style.gridRow = "2 / span 1";
  item.style.transform = `translateY(${start * pixelsPerMinute}px)`;
  item.style.height = `${duration * pixelsPerMinute}px`;
